...
class State(np.ndarray):
    ...
    def __new__(cls, *args):
        ...
        if len(args) == 1:
            ...
            obj = np.asarray(args[0]).view(cls)  # normal numpy array reclassed as State type

        elif len(args) == 4:
            assert not [True for arg in args if not is_float_or_int(arg)], "Arguments must be ints or floats"
            obj = np.asarray(args).view(cls)  # normal numpy array reclasssed as State type
